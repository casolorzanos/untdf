...
divAdjust = '@@@% población/comunidad infectada inicialmente<br/><font style="color:grey; font-size: 1.2vmax;"> (En el caso de comunidades, @@@% de la comunidad es infectada inicialmente.)</font>'
if (dmode == 'landscape') {
	divAdjust = '@@@% población/comunidad infectada inicialmente<br/><font style="color:grey; font-size: 0.8vmax;">(En el caso de comunidades, @@@% de la comunidad es infectada inicialmente.)</font>'
}
parameterData[2] = {name: 'fractionInfectedInitially', div: divAdjust, initValue: 4, max: 100, min: 0, step: 0.1, color: '10, 100%, 63%', transform: 'parseFloat(#paraValue#)/100', requiresReset: true, runFAtEnd: ''}
...
divAdjust = 'Distanciamiento social<br/><font style="color:grey; font-size: 0.8vmax;">El desplazamiento hacia la derecha aumenta el distaciamiento social entre individuos.</font>'
parameterData[4] = {name: 'socialDistancingFactorA', div: divAdjust,initValue: 0, max: 1, min: 0, step: 0.01, color: '40, 100%, 50%', transform: 'parseFloat(#paraValue#)', requiresReset: false, runFAtEnd: 'boundaryForceC = linearValue(0, 1, 0.1, 2,simulationParameters.socialDistancingFactorA)'}
parameterData[5] = {name: 'socialDistanceObedientPop', div: '@@@% de la población obedece el distanciamiento social',initValue: 100, max: 100, min: 0, step: 0.1, color: '40, 100%, 50%', transform: 'parseFloat(#paraValue#)/100', requiresReset: false, runFAtEnd: 'handleSDObedience()'}

divAdjust = 'Se observa distanciamiento social dentro de @@@ veces el radio de infección.<br/><font style="color:grey; font-size: 1.2vmax;"> (Esta cantidad reduce muchos cálculos. Mientras más chico el valor, mejor. Pero, valores grandes pueden imitar la simulación del video)</font>'
if (dmode == 'landscape') {
	divAdjust = 'Se observa distanciamiento social dentro de @@@ veces el radio de infección.<br/><font style="color:grey; font-size: 0.8vmax;">(Esta cantidad reduce muchos cálculos. Mientras más chico el valor, mejor. Pero, valores grandes pueden imitar la simulación del video)</font>'
}
...
parameterData[7] = {name: 'quarantineAfter', div: 'El sistema de salud detecta un individuo contagiado @@@ días después de producido el contagio.',initValue: 7, max: 25, min: 1, step: 1, color: '10, 100%, 63%', transform: 'parseInt(#paraValue#)', requiresReset: true, runFAtEnd: ''}
parameterData[8] = {name: 'startQ', div: 'El sistema de salud se pone en marcha @@@ días después del comienzo de la epidemia.',initValue: 2, max: 50, min: 0, step: 1, color: '10, 100%, 63%', transform: 'parseInt(#paraValue#)', requiresReset: true, runFAtEnd: ''}
parameterData[9] = {name: 'probabilityOfNoSymptoms', div: "@@@% de los contagiados son asintomáticos, luego, no son detectados por el sistema de salud.", initValue: 20, max: 100, min: 0, step: 0.1, color: '58, 100%, 50%', transform: 'parseFloat(#paraValue#)/100', requiresReset: false, runFAtEnd: ''}
...
//parameterData[14] = {name: 'numberOfParticles', div: 'Número de individuos &nbsp&nbsp @@@' ,initValue: 200, max: 2000, min: 10, step: 1, color: '198, 100%, 50%', transform: 'parseInt(#paraValue#)', requiresReset: true, runFAtEnd: ''}
//parameterData[15] = {name: 'particleSize', div: 'Tamaño de los individuos &nbsp&nbsp @@@',initValue: 0.7, max: 1, min: 0.1, step: 0.001, color: '198, 100%, 50%', transform: 'parseFloat(#paraValue#)', requiresReset: true, runFAtEnd: ''}
...
divAdjust = '@@@ días simulados por segundo <br/><font style="color:grey; font-size: 1.2vmax;"> (Puede variar dependiendo de la velocidad de tu reloj.)</font>'
if (dmode == 'landscape') {
	divAdjust = '@@@ días simulados por segundo <br/><font style="color:grey; font-size: 0.8vmax;"> (Puede variar dependiendo de la velocidad de tu reloj.)</font>'
}
	...
	if (parData.requiresReset) {
		if (currentSim == 'simpleCase') {
			...
			clearInterval(loopinterval);
	 		...
	 		timecount = 0
	 		loopcount = 0
			removeParticlesFromBoundary('centralLocationQuarantineCaseGraph', 'centralLocationQuarantineCaseB')
			removeParticlesFromBoundary('centralLocationQuarantineCaseGraph', 'centralLocationQuarantineB')
			...
			clearInterval(loopinterval);
	 		...
	 		for (bdi = 0; bdi < 3; bdi++) {
				for (bdj = 1; bdj < 2; bdj++) {
					removeParticlesFromBoundary('communitiesCaseGraph', 'communitiesCaseB_' + bdi + '_' + bdj)
				}
			}
			...
			clearInterval(loopinterval);
	 		...
	 		for (bdi = 0; bdi < 3; bdi++) {
				for (bdj = 1; bdj < 2; bdj++) {
					removeParticlesFromBoundary('communitiesQuarantineCaseGraph', 'communitiesQuarantineCaseB_' + bdi + '_' + bdj)
				}
			}
			removeParticlesFromBoundary('communitiesQuarantineCaseGraph', 'communitiesQuarantineB')
...
stateColors = {'infected':'10, 100%,70%','susceptible': '195, 100%, 100%', 'removed':'195, 100%, 50%', 'noSymptoms' : '58, 100%, 50%'}
...
function setUpSimpleCaseGraph() {
	...
	addParticlesToBoundary('simpleCaseGraphG', 'simpleCaseGraphB', 200)
...
function setUpSimpleQuarantineCaseGraph() {
	...
	addParticlesToBoundary('simpleCaseGraph&QG', 'simpleCaseGraph&QB', 200)
...
function setUpCentralLocationCaseGraph() {
	...
	addParticlesToBoundary('centralLocationCaseG', 'centralLocationCaseB', 200)
...
function setUpCentralLocationQuarantineCaseGraph() {
	...
	addParticlesToBoundary('centralLocationQuarantineCaseGraph', 'centralLocationQuarantineCaseB', 200)
...
function setUpCommunitiesCaseGraph() {
	...
	communitiesToInfect = communitiesToInfect.slice(0, 2)
	...
	for (bdi = 0; bdi < 3; bdi++) {
		for (bdj = 1; bdj < 2; bdj++) {
			...
			addParticlesToBoundary('communitiesCaseGraph', 'communitiesCaseB_' + bdi + '_' + bdj, 60)
...
function setUpCommunitiesQuarantineCaseGraph() {
	...
	communitiesQuarantineToInfect = communitiesQuarantineToInfect.slice(0, 2)
	...
	for (bdi = 0; bdi < 3; bdi++) {
		for (bdj = 1; bdj < 2; bdj++) {
			...
			addParticlesToBoundary('communitiesQuarantineCaseGraph', 'communitiesQuarantineCaseB_' + bdi + '_' + bdj,60)
...
function moveParticles(ofBoundary) {
	...
	for (var particleIndex in partDB) {
		...
		if ((currentSim == 'centralLocationCase' || currentSim == 'centralLocationQuarantineCase') && particleData[ofBoundary][particleIndex].moveAnimation == false && Math.random() < (0.4/100) && particleData[ofBoundary][particleIndex].visitingCentralLocation == false) {
...
function animateParticles(ofBoundary) {
	...
	for (var particleIndex in partDB) {
		...
		if (particleData[ofBoundary][particleIndex].infectionAnimation) {
			particleData[ofBoundary][particleIndex].infectionAnimationFrame = particleData[ofBoundary][particleIndex].infectionAnimationFrame + (100*3/(6*40))
...
function interactions(ofBoundary) {
	...
	for (var particleIndex in partDB) {
		...
		if (particleData[ofBoundary][mainParticle].state == 'infected' && particleData[ofBoundary][mainParticle].moveAnimation == false) {
			...
			if (particleData[ofBoundary][mainParticle].daysSinceInfection > 25) {
		...
		if (currentSim == 'communitiesCase' && particleData[ofBoundary][mainParticle].moveAnimation == false && Math.random() < (2/100)) {
		...
		if (particleData[ofBoundary][mainParticle] != undefined) {
			if (currentSim == 'communitiesQuarantineCase' && particleData[ofBoundary][mainParticle].moveAnimation == false && Math.random() < (2/100) && particleData[ofBoundary][mainParticle].quarantined == false) {
...
function updatePlotGraph() {
	...
	options = {}
	options.text = removedV.toFixed(1) + '% removidos'
...
function setAnimationDetails() {
	...
	calculationFrameInterval = (40/(timeStepsPerDay*3))*frameInterval
}
...
function resetSimHard(event) {
	if (currentSim == 'simpleCase') {
		...
		clearInterval(loopinterval);
 		clearInterval(loopinterval2);
 		changeParameterWithoutReload(10, 0.1)
 		changeParameterWithoutReload(2, 4)
 		changeParameterWithoutReload(7, 7)
 		changeParameterWithoutReload(8, 2)
		...
		clearInterval(loopinterval);
 		clearInterval(loopinterval2);
 		changeParameterWithoutReload(10, 0.2)
 		changeParameterWithoutReload(2, 4)
 		changeParameterWithoutReload(7, 7)
 		changeParameterWithoutReload(8, 2)
 		changeParameterWithoutReload(9, 20)
...
graphToQMap = {}
graphToQMap['simpleCase'] = ['simpleCase&Q', 'simpleCaseGraph&QG', 'simpleCaseGraph&QH', 'simpleCaseGraphH', 'Caso Simple'] 
graphToQMap['simpleCase&Q'] = ['simpleCase', 'simpleCaseGraphG', 'simpleCaseGraphH', 'simpleCaseGraph&QH', 'Caso Simple con Medidas del Sistema de Salud']

graphToQMap['centralLocationQuarantineCase'] = ['centralLocationCase', 'centralLocationCaseG', 'centralLocationCaseGraphH', 'centralLocationQuarantineCaseGraphH', 'Punto de Aglomeración con Medidas del Sistema de Salud']
graphToQMap['centralLocationCase'] = ['centralLocationQuarantineCase', 'centralLocationQuarantineCaseGraph', 'centralLocationQuarantineCaseGraphH', 'centralLocationCaseGraphH', 'Punto de Aglomeración']

graphToQMap['communitiesQuarantineCase'] = ['communitiesCase', 'communitiesCaseGraph', 'communitiesCaseGraphH', 'communitiesQuarantineCaseGraphH', 'Interacción entre Comunidades con Medidas del Sistema de Salud']
graphToQMap['communitiesCase'] = ['communitiesQuarantineCase', 'communitiesQuarantineCaseGraph', 'communitiesQuarantineCaseGraphH', 'communitiesCaseGraphH', 'Interacción entre Comunidades']
...
function handleConfigChange(event) {
	...
	document.getElementById('centralLocationCaseGraphH').style.visibility = 'hidden'
	document.getElementById('centralLocationQuarantineCaseGraphH').style.visibility = 'hidden'
	document.getElementById('communitiesCaseGraphH').style.visibility = 'hidden'
	document.getElementById('communitiesQuarantineCaseGraphH').style.visibility = 'hidden'

	document.getElementById(handleCases[event.target.id][quarantineOn][0]).style.visibility = 'visible'
	document.getElementById(handleCases[event.target.id][quarantineOn][0]).style.zIndex = 100
	...
	document.getElementById('simpleCaseoption').style.border = '1px solid hsla(0, 0%, 60%, 1)'
	document.getElementById('centralLocationoption').style.border = '1px solid hsla(0, 0%, 60%, 1)'
	document.getElementById('communitiesoption').style.border = '1px solid hsla(0, 0%, 60%, 1)'
	...
	document.getElementById('simpleCaseoption').style.color = 'hsla(1, 0%, 60%, 1)'
	document.getElementById('centralLocationoption').style.color = 'hsla(1, 0%, 60%, 1)'
	document.getElementById('communitiesoption').style.color = 'hsla(1, 0%, 60%, 1)'
	...
	document.getElementById('caseTypeName').innerHTML = graphToQMap[currentSim][4]
}
