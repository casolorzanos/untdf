...
function parameterEvent(event) {
  ...
  if (pName == "quarantineAfterdiv") {
    updateHealthActionsDescription(event.target.value);
  }
...
simulationParameters.infectionRadius = 0.13;
...
function resetSimHard(event) {
  if (currentSim == "simpleCase") {
    ...
    clearInterval(loopinterval);
    clearInterval(loopinterval2);
    simulationParameters.infectionRadius = 0.23;
...
graphToQMap = {};
updateHealthActionsDescription(parameterData[7].initValue);

quarantineOn = false;

function updateHealthActionsDescription(value) {
